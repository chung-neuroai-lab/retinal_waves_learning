...
from src.mftma.local_linear_centers import localLinCenters, sampleDiff
...
def manifold_analysis_corr_gaussian_width(XtotT, kappa, n_t, t_vecs=None, n_reps=10):
    '''
    ...
    for i in range(num_manifolds):
        ...
        R_M, D_M = compute_gauss_RD(S_r, n_t)

        # Store the results
        R_M_vec[i] = R_M
        D_M_vec[i] = D_M

    return R_M_vec, D_M_vec


def manifold_analysis_corr(XtotT, kappa, n_t, t_vecs=None, n_reps=10, use_sparse_capacity=False, num_of_manifolds=10):
    '''
    Carry out the analysis on multiple manifolds.
    Args:
        XtotT: Sequence of 2D arrays of shape (N, P_i) where N is the dimensionality
    ...
    if use_sparse_capacity:
        print("use_sparse_capacity!")
        LLEMethod = "nn"
        samp_n = num_of_manifolds+5

        # Get CLLD examplars for computing low rank structure 
        centers = localLinCenters(XtotT,method=LLEMethod)
        examplars = sampleDiff(centers, samp_n = samp_n, method='center')
        examplars = np.stack(examplars, axis=1) # Centers is of shape (N, m) for m manifolds
        examplar_mean = np.mean(examplars, axis=1, keepdims=True) # (N, 1) mean of all centers
        centers = np.stack(centers, axis=1) # Centers is of shape (N, m) for m manifolds
        
        UU, SS, VV = np.linalg.svd(examplars - examplar_mean)
    else:
        print("not use_sparse_capacity!")
        # Compute the mean for each manifold
        centers = [np.mean(XtotT[i], axis=1) for i in range(num_manifolds)]
        centers = np.stack(centers, axis=1) # Centers is of shape (N, m) for m manifolds
        center_mean = np.mean(centers, axis=1, keepdims=True) # (N, 1) mean of all centers
...
def compute_gauss_RD(S, n_t):
    N, m = S.shape
    t_vec = np.random.randn(N, n_t)
    smax = np.zeros((N,n_t))
    smax_norm_all = []
    tShat_all = []
    for i in range (n_t):
        ...
        t = t.reshape(-1,1) 
        #print("t",t.shape)
        A = np.matmul(t.T,S) #(1xn)(nXm)
        tSmax = A.max()
        imax = A.argmax()
        #print(tSmax,imax)
        smax = S[:,imax]
        smax_norm_all.append(np.linalg.norm(smax,2))
        tShat_all.append(np.matmul(t.T,smax/np.linalg.norm(smax,2)))
    Rg_sq = np.mean(np.square(smax_norm_all))
    Rg = np.sqrt(Rg_sq)
    Dg = np.mean(np.square(tShat_all))
    return Rg, Dg

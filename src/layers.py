...
class Retina(object):
    def __init__(self, n_neurons=3200, grid_length=32, discrete=True):
        ...
        if discrete == True:
            self.n_neurons = self.grid_length**2
            self.pos = np.array(list(itertools.product(np.arange(self.grid_length),np.arange(self.grid_length))))
    ...
    def prop_wave(self, return_fired_mask=True):
        ...
        #update voltage
        self.voltage += 0.5*(0.04*self.voltage**2 + 5*self.voltage + 140 - self.total_leak + self.total_gain)
        self.voltage += 0.5*(0.04*self.voltage**2 + 5*self.voltage + 140 - self.total_leak + self.total_gain)
...
class LGN(object):
    ...
    def __init__(self, 
        ...
        elif weight_dist == 'logn':
            logn_mean = np.log(weight_mu**2/(np.sqrt(weight_mu**2+weight_sig**2)))
            logn_var = np.log(1+weight_sig**2/weight_mu**2)
            self.weights = torch.distributions.log_normal.LogNormal(logn_mean, np.sqrt(logn_var)).sample((n_neurons, n_retinal_neurons)).to(device)
    ...
    def pretrain(self, retina, timesteps=1000000, eta=0.1, plot_every=10000, track_every=None, data=None):
        ...
        if data is not None:
            timesteps = data.shape[-1] # 72000

        for t in range(timesteps):
            if data is not None:
                #for training on real data images
                wave = data[:,t][:,None].type(torch.float) # torch.Size([1024, 1])
            else:
    ...
    def max_pool_forward_pass(self, wave):
        ...
        weighted_inputs = self.lgn_forward_pass(wave)
        
        #get the activated unit
        activations = self.get_winner(self.ReLU(weighted_inputs))
        return activations

    def lgn_forward_pass(self, batch, sparse=0):
        out = self.ReLU(torch.matmul(self.weights, batch))
        if sparse:
            last_out = torch.sort(out, dim=0, descending=True).values[sparse,:]
            out[(out <= last_out.repeat(self.n_neurons,1))] = 0
            # last_out = torch.randperm(400)[:sparse]
            # out = out[last_out,:]
        return(out)
...
class FunctionalLGN(LGN):
    ...
    def __init__(self, 
        ...
        if svm_output == True:
            self.svm_classifier = svm.LinearSVC(dual=False)
    ...
    def train_svm(self, batch, targets, hidden=False, pool=False, sparse=None):
        ...
        """
        if hidden:
            rep = self.hidden_forward_pass(batch, pool)
        
        else:
            rep = self.lgn_forward_pass(batch, sparse=sparse)

        self.svm_classifier.fit(rep.T, targets)
    

    def evaluate_svm_performance(self, batch, targets, hidden=False, pool=False, sparse=None):
        ...
        predicted_targets = self.svm_classifier.predict(np.array(rep.T))

        #evaluate loss and accuracy
        # loss = hinge_loss(targets, predicted_targets)
        return(accuracy_score(targets, predicted_targets))

    ...
    def project_data_onto_retina(self, 
        ...
        if self.targets is not None:
            ...
            for i in class_idx_list:
                x.append(i.item())
            imageset = self.raw_data[x,:,:]
            self.targets = self.targets[x]
        ...
        for idx, image in enumerate(imageset): 
            ...
            for cell, xpos, ypos in zip(range(n_neurons), self.retina.pos[:,1], self.retina.pos[:,0]):
                ...
                if binarize == True:
                    if image[raw_ypos, raw_xpos] > image.mean() + threshold_stds*image.std():
                        retinal_imageset[cell, idx] = 1
                
                else:
                    retinal_imageset[cell, idx] = image[raw_ypos, raw_xpos]
